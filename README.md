...
python3 scripts/build_defect_sheet.py   # (re)builds the Defect sheet in the xlsx, cross-referencing each defect's related TC IDs against their current Status
...
| DEF-API-001 | API layer | `/login`, `/purchaseorder`, `/viewcart`, `/addtocart`, `/deleteitem` return HTTP 200 for almost every malformed or invalid request instead of 400/401/404 - no input validation, no auth enforcement at the API layer. Documented separately in `docs/api-analysis/BUG_REPORT_API_FAILURES.md` with the full request/response matrix. | Critical (API contract) |
| DEF-TEST-001 | Test infrastructure (not an app defect) | The 13 automated cases for DEF-001/002/003/004 were hanging for ~96-100s each (waiting on a validation alert that never fires) instead of failing in seconds. Fixed - see limitations below. | Low |

## Known limitations & next steps

- **TC-CRT-046 (guest cart leak) unexpectedly passed in the last recorded run.** The 2026-08-01 06:48 UTC run (`reports/test-results/archive/2026-08-01-run-001/results.json`) shows this `test.fail()`-annotated case coming back green in 4.6s instead of reproducing the defect, while `TC-CRT-047` (modal stacking) and the card/month/year cases all failed as expected in the same run. This doesn't necessarily mean DEF-SYS-001 is fixed - the shared-bucket bug depends on other guests' live traffic, so a 1-item cart is possible by chance. Needs a manual re-check before trusting either outcome.
- **The card/month/year negative cases (DEF-001/002/003/004) were hanging for ~96-100s instead of failing fast.** They *are* redefined with correct reject-based assertions and `test.fail()` annotations (`cart.spec.ts` TC-CRT-020/027-038/023b), but `clickPurchaseExpectingAlert()` waits indefinitely for a native validation alert - since Demoblaze has no client-side validation for these fields at all, that alert never comes, so every one of these 13 cases ran to the full config timeout before failing. Fixed by adding `CartPage.clickPurchaseExpectingAlertOrAccept()`, which races the alert against the SweetAlert success popup with an 8s bound. The fix is in this commit; the `results.json` evidence currently referenced in the Defect sheet still reflects the pre-fix (slow but correct-outcome) run - re-running `npm run test:manual-suite` will produce clean, fast evidence.
- **Cross-browser is configured, not yet proven stable.** Firefox and WebKit were only just added to `playwright.config.ts`. WebKit is known to need the retry-on-submit workaround already present in `regression.spec.ts`; that workaround hasn't been ported to `LoginPage`/`login.spec.ts` yet - deliberately held off while `LoginPage.ts`/`login.spec.ts` are under active investigation (see the TC-LOG-015/016/017 item below), to avoid two people/passes touching the same file at once. So a full 3-browser CI run may need another pass to be green everywhere.
- **TC-LOG-015/016/017 (empty username/password) hang to the full test timeout instead of failing fast.** Confirmed fresh (not stale) in the 2026-08-01 07:45 UTC run, which post-dates the `LoginPage.login()` race fix. By that fix's own logic these should fail within ~10-15s once both the dialog-wait and welcome-banner-wait time out, so this is an open, not-yet-understood regression, currently under investigation.
...
- **The xlsx Status column now uses proper Pass/Fail/Flaky/Skipped values**, synced from `reports/test-results/archive/2026-08-01-run-001/results.json` via `sync_test_results_to_xlsx.py` (an earlier version of that script wrote Playwright's internal `expected`/`unexpected` vocabulary straight into the Status column - fixed). A fresh `npm run test:manual-suite && npm run sync:tc-sheet && python3 scripts/build_defect_sheet.py` after the DEF-TEST-001 fix above would give cleaner timing data end-to-end.
