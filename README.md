...
| DEF-API-004 | API / `/view` | Product details endpoint doesn't validate the product ID - non-existent ID, missing `id`, and `id=0` all return 200 instead of 400/404. | High |
...
| DEF-API-007 | API / `/addtocart` persistence | `/addtocart` returns HTTP 200 for two sequential adds using a synthetic cookie (never seen by `index.js`), but the following `/viewcart` reports 0 items instead of 2 - silent data loss behind a misleading success response. Confirmed as a real defect, not test flakiness: 9/9 identical failures across 3 browser engines x `--repeat-each=3` rules out a client-side race. Likely the same root property as DEF-SYS-001 (only `index.js` establishes a "real" guest identity). | High |
...
| DEF-TEST-002 | Test infrastructure (not an app defect) | `API-CART-001`/`API-DEL-004` asserted a made-up `{status, data:{items}}` shape for `/viewcart` instead of the real `{Items:[...]}`. Confirmed fixed. | Low |
| DEF-TEST-004 | Test infrastructure (not an app defect) - fix applied, unverified | `API-PROD-001`/`002` asserted product ID as the `idp_` URL-param name instead of the real `id` request-body field (two different products both failing identically pointed at a request-shape bug, not missing catalog data). Fixed in `tests/api.spec.ts`, not yet re-confirmed with a live run. | Low |

16 defects total: 13 confirmed application defects, 3 test-script bugs/findings caught during this work (2 confirmed fixed, 1 fix applied but unverified - see the Summary sheet's "Defect Summary" block for the live count). Note: what started as "`API-ADD-002` might be a test-script race condition" (an early, tentatively-labeled `DEF-TEST-003`) was reclassified as the real `DEF-API-007` above once repeat testing across browser engines ruled out flakiness - the Defect sheet is the source of truth if this table and it ever disagree.
